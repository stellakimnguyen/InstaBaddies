...
exports.createNotificationOnPost = functions
  ...
  .onCreate((snapshot) => { // snapshot is data for the post just made for user20
    return db
      .doc(`/followers/${snapshot.data().username}`)
      .get()
      .then((doc) => { // doc is data of followers for the user20
        if (doc.exists) {
          ...
          for (const key in data) {
            ...
            let docRef = db
              .doc(`/notifications/${snapshot.id}-${Math.round(Math.random() * 100000)}`)
      ...
      .then(() => {
        res.json({ message: "Notifications created successfully for new Post" });
      })
      .catch((err) => {
        console.error(err);
        return;
