...
exports.signup = (req, res) => {
  ...
  db
    ...
    .get()
    .then(doc => { // then block MUST return smt
...
exports.getUserDetails = (req, res) => {
  ...
  db.doc(`/users/${req.params.username}`).get()
    ...
    .then(data => { // data is an array of documents matching the query above : posts with corresponding username
      userData.posts = [];
      data.forEach(doc => {
        userData.posts.push({
...
exports.getUserDetailsAndFollowing = (req, res) => {
  ...
  db.doc(`/users/${req.params.username}`).get()
    ...
    .then(data => { // data is an array of documents matching the query above : posts with corresponding username
      ...
      ///////////////////////////////
      return db
        .doc(`/following/${req.user.username}`).get() // look for doc for user that is logged in in following collection
  ...
  db.doc(`/users/${req.user.username}`).get()
    ...
    .then(data => {
      ...
      return db.collection('notifications').where('recipient', '==', req.user.username)
        .orderBy('createdAt', 'desc').get(); // returns a promise with data
    })
    .then(data => { // data is an array of documents that matched above query from notifications collection
      userData.notifications = [];
      data.forEach(doc => {
        userData.notifications.push({ // push all fields from the notification document one by one
...
exports.followUser = (req, res) => {
  const followingRef = db.collection('following').doc(`${req.params.signedInAs}`);
