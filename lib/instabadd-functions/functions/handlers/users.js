...
exports.login = (req, res) => {
  console.log('we are in login in users handler');
  const user = {
    email: req.body.email,
    password: req.body.password
  };
...
exports.addUserDetails = (req, res) => {
  ...
  console.log('we are in addUserDetails');
...
exports.getAllUsers = (req, res) => {
  db.collection("users")
    .orderBy("username") // order by name, alphabetical (default value for second arg)
    .get()
    .then(data => {
      let usersList = [];
      // data is an attribute that is an array of docs
      data.forEach(doc => {
        // Get data from ref 'doc' and populate usersList
        usersList.push({
          username: doc.data().username,
...
exports.getUserDetailsAndFollowing = (req, res) => {
  console.log("We are in getUserDetailsAndFollowing");
  let userData = {};
  // Find username in DB then extract that user's document info
  db.doc(`/users/${req.params.username}`)
    .get()
...
exports.getAuthenticatedUser = (req, res) => {
  console.log('we are in getAuthenticatedUser');
  let userData = {};
  ...
  db.doc(`/users/${req.user.username}`)
    ...
    .then(data => {
      ...
      return db.doc(`/following/${req.user.username}`).get();
    })
    .then(doc => {
      if (doc.exists) {
        userData.following = doc.data();
      }
      return res.json(userData);
    })
    .catch(err => {
...
exports.followUser = (req, res) => {
  let followingRef = db.collection("following").doc(`${req.params.signedInAs}`);
  let followersRef = db.collection("followers").doc(`${req.params.username}`);
  ...
  followingRef
    ...
    .then(() => {
      ...
      if (process.env.NODE_TEST === "unitTest") {
        ...
        // console.log("following ref : ", db.collection("following"));
        followingRef = db.collection("following");
      }
      followingRef.set(
        {
